...
export const createMenu = <LevelDatas>(qwick: Qwick, graphics: Graphics, game: Game<LevelDatas>) => {
    ...
    const startButton = createButton(
        ...
        [hasLevelEditor ? -0.15 : 0, -0.2],
        [0.12, 0.04],
        "Start"
    );
    const editorButton = createButton(qwick.input.getMousePos, [0.15, -0.2], [0.12, 0.04], "Level Editor");
    ...
    const levelButtons = game.levels.map((_, i) => {
        ...
        return createButton(qwick.input.getMousePos, [x, y], [0.1, 0.04], `${i + 1}`);
    });
